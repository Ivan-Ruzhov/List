...
class TodoApp extends Component {
  ...
  onLabel = (id) => {
    this.setState(({ arrTask }) => {
      ...
      const newArr = [...this.state.arrTask.slice(0, idx), newItem, ...this.state.arrTask.slice(idx + 1)]
      return {
        arrTask: newArr,
        filterTask: newArr,
      }
  ...
  onDelete = (id) => {
    this.setState(({ arrTask }) => {
      const idx = arrTask.findIndex((el) => el.id === id)
      const newArr = [...this.state.arrTask.slice(0, idx), ...this.state.arrTask.slice(idx + 1)]
      return {
        arrTask: newArr,
        filterTask: newArr,
      }
